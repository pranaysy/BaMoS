...
PathScratch=/scratch0/csudre/${ID}_BaMoSPreProc_${Opt}_${JOB_ID}
...
echo "echo "Reorientation to standard space of T1 image" " >> ${NameScript}
    ...
    echo "${PathSeg}/seg_maths ${PathScratch}/T1_${ID}.nii.gz -odt float ${PathScratch}/T1_${ID}.nii.gz" >> ${NameScript}

    echo "echo "Alignment of FLAIR image to T1"" >> ${NameScript}
    echo "${PathReg}/reg_aladin -ref ${PathScratch}/T1_${ID}.nii.gz -flo ${ImageFLAIR} -aff ${PathScratch}/Aff_FLAIRtoT1.txt -res ${PathScratch}/FLAIR_${ID}.nii.gz -rigOnly" >> ${NameScript}
...
echo "echo "Alignment of T1 image to FLAIR"" >> ${NameScript}
    echo "${PathReg}/reg_aladin -ref ${PathScratch}/FLAIR_${ID}.nii.gz -flo ${ImageT1} -aff ${PathScratch}/Aff_T1toFLAIR.txt -res ${PathScratch}/T1_${ID}.nii.gz -rigOnly" >> ${NameScript}
    echo "${PathSeg}/seg_maths ${PathScratch}/T1_${ID}.nii.gz -odt float ${PathScratch}/T1_${ID}.nii.gz" >> ${NameScript}
for p in Segmentation prior TIV	Parcellation
do
echo "${PathReg}/reg_resample -inter 1 -flo  ${PathGIF}/*${p}* -res ${PathScratch}/GIF_${ID}_${p}.nii.gz -ref ${PathScratch}/FLAIR_${ID}.nii.gz -aff ${PathScratch}/Aff_T1toFLAIR.txt" >> ${NameScript}
echo "${PathReg}/reg_resample -inter 1 -flo  ${PathGIF}/*${p}* -res ${PathScratch}/GIF_${p}_${ID}.nii.gz -ref ${PathScratch}/FLAIR_${ID}.nii.gz -aff ${PathScratch}/Aff_T1toFLAIR.txt" >> ${NameScript}
done
...
qsub -l h_rt=1:0:0 -l tmem=${Mem}G -l h_vmem=${Mem}G -N BaMoSLes${ID} ${NameScript}	
